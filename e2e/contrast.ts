 ...
 *    exactly that: `<body>` carries TWO `radial-gradient(circle at …)` washes,
 *    one `--accent` at 28% and one `--accent-2` at 22%, over the flat `--bg`;
 *    and every panel on top of them is TRANSLUCENT — `--panel` is
 *    `rgba(13,25,31,0.88)` in dark and `rgba(255,255,255,0.96)` in light, with
 *    `--panel-soft` at `0.92`/`0.97`. So the real surface under any given word
 *    depends on where on the page that word sits, which is why this walk samples
 *    each gradient at the text's own coordinates rather than judging it at its
 *    worst stop. The hero aside adds a `color-mix(in oklab, var(--accent) 6%,
 *    transparent)`, and the shared top bar's `--cl-ink` is another `color-mix`.
 *    axe files all of them under `incomplete`.
 ...
 *    at `.85`, `button:disabled` at `.55`, and the page `<footer>`, whose inline
 *    `style="opacity: 0.8"` fades its prose AND its five related-demo links.
 *
 ...
 *    box is `.chain-wrap`, which wraps the recomputed challenge chain and grows
 *    one `<span class="chain">` per ring member, and the `.sr-only` recipe
 *    handled separately below. Content scrolled past a clipper is not dimmed or
 ...
 *    never laid down.
 *
 *  - SVG PAINTS IN DOCUMENT ORDER, SO SIBLINGS CAN BE THE BACKGROUND — and the
 *    stroke-only guard inside `svgUnderlay` is load-bearing here rather than
 *    defensive. Exhibit 3's `.scatter` chart is a real `<svg>` whose axis ticks
 *    and titles are `<text>` elements drawn AFTER the gridlines, the axes, the
 *    reference line and both series polylines. Every one of those is a
 *    stroke-only `<line>` or `<polyline>` whose computed `fill` is SVG's initial
 *    BLACK, so an unguarded sibling walk would composite an opaque black
 *    rectangle under half the labels on the chart and invent failures for text
 *    whose real backdrop is the translucent panel.
 *
 ...
 * not one — nor is it one to axe's `color-contrast` rule. This page has one that
 * carries meaning: `.explainer summary::before { content: '▸' }`, rotated 90deg
 * when the disclosure is open. The rule that draws it also sets
 * `list-style: none` and hides `::-webkit-details-marker`, so the user agent's
 * own triangle is gone and this glyph is the ENTIRE affordance saying the four
 * mechanism explainers can be expanded. `nontext.ts` measures it, from `scan()`,
 * in every driven state.
 */
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven pass here walks five exhibits, a ring of up to eleven nodes with
     * an SVG edge per member, one response chip and one challenge span per ring
     * member, a growing ledger and a scatter chart with a tick per sample — and
     * `render()` rebuilds `#app` wholesale on every state change, so nothing can
     * be cached across renders. Within one pass the caches still save tens of
     * thousands of redundant `getComputedStyle` calls. Nothing mutates the DOM
     ...
     * that is `.chain-wrap` and the `.sr-only` boxes, the latter handled
     * separately below.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // ratio was 6.15:1. BOTH branches are live on this page. Exhibit 3's
      // `.scatter` draws its gridlines, both axes, the dashed reference line and
      // two series polylines BEFORE its `<text>` ticks and titles, and every one
      // of those is stroke-only with an inherited black `fill` — precisely the
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // `<details class="explainer">` — the four mechanism explainers, one under
      // each exhibit, all of which ship shut. The gate opens each by clicking
      // its <summary>, which is the route a reader has, rather than setting
      // `.open = true` from script — which is exactly what the gate this
      ...
      // markup carries SIX of them: a `<legend class="sr-only">` naming each of
      // the four `fieldset.controls-row`s, the `<figcaption>` describing the
      // scatter chart, and `#open-desc`, which is the `aria-describedby` target
      // for the manager-open button. Every one has a rect and passes
      ...
      // narrow: only a ZERO-AREA clip qualifies, so a partially clipped element
      // is still measured, worst case.
     ...
     * or duplicated: the `.badge` chip beside each exhibit heading (whose text
     * restates the heading), the `→` `.chain-arrow`s between challenge values,
     ...
     *
     * The guard matters here because the usual way the mistake arrives is
     * exactly how this lab builds its DOM: `render()` is one enormous
     * `innerHTML` template literal, and both SVG figures are assembled by
     * `.map(...).join('')` inside it. Forget the `join` and JS inserts commas
     * between array entries, leaving a run of "," text nodes inside the `<svg>`
     * — which render nothing, while a walk that measured them would report a
     ...
     *
     * It is live here and the stylesheet says so in as many words: `html` sets
     * `background-color: var(--bg)` with the comment "Own background-color so
     * WebKit does not promote body's background to the canvas and report body as
     * transparent (axe would then assume a white page and flag all dark-theme
     * text)". That fix is for axe; this branch is the same fix for the
     * arithmetic walk. `body` is `min-height: 100vh` while the document runs
     * several viewports, so without it everything below the fold would composite
     * onto WHITE in a lab whose entire dark palette is near-black.
     */
